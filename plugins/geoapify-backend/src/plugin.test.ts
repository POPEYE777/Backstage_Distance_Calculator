...
import { geoapifyPlugin } from './plugin';
...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    const { server } = await startTestBackend({
      features: [geoapifyPlugin],
    });

    await request(server).get('/api/geoapify/todos').expect(200, {
      items: [],
    });

    const createRes = await request(server)
      .post('/api/geoapify/todos')
      .send({ title: 'My Todo' });
    ...
    await request(server)
      .get('/api/geoapify/todos')
      .expect(200, {
        items: [createdTodoItem],
      });

    await request(server)
      .get(`/api/geoapify/todos/${createdTodoItem.id}`)
